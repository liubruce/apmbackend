...
//        console.log('loadAppStatic begin');
        var self = this;
        for(var i = 0, l = plugins.length; i < l; i++){
            try{
                var plugin = require("./"+plugins[i]+"/frontend/app");
                plugs.push(plugin);
//                app.use(apmConfig.path + '/'+plugins[i], express.static(__dirname+ '/'+plugins[i]+"/frontend/public", { maxAge:31557600000 })); //,{ maxAge:31557600000 } apmConfig.path+'/'+plugins[i]
    ...
    this.singleDefaultConnection = function() {
        var conf = Object.assign({}, apmConfig.mongodb);
        for (var k in conf) {
            if (typeof k === 'object') {
